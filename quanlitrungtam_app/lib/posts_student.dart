...
class StudentPostsScreen extends StatefulWidget{
  var roomID;
  StudentPostsScreen({Key? key, @required this.roomID}) : super(key: key);

  @override
  State<StudentPostsScreen> createState() => _StudentPostsScreenState(roomID);
}

class _StudentPostsScreenState extends State<StudentPostsScreen>{
  final roomID;
  _StudentPostsScreenState(this.roomID);
  String _email = '';
  String _password = '';
  String _type = '1';
  ...
  loadPostList() async {
    ...
    await http
        .get(Uri.parse('http://10.0.2.2:8000/api/post1list' + params.toString()))
        .then((response){
